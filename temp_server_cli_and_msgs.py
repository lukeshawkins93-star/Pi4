...
def collect_data(elapsed_minutes):
    ...
    print(f"MEAT(F):{round(var1+cal1,1)} AIR(F):{round(var2+cal2,1)} Time(Min):{round(elapsed_minutes,2)}", end="\r", flush=True)
    ...
    if ((elapsed_minutes-timer)>1):
        timer=elapsed_minutes
        send_message(f"MEAT(F):{round(var1+cal1,1)} AIR(F):{round(var2+cal2,1)} Time(Min):{round(elapsed_minutes,2)}")
